...
- Updated CI and transitive dependency resolutions, including patched `@hono/node-server`, `fast-uri`, `ip-address`, and `undici` releases.
...
- Reduced OpenClaw Quick Replies evaluator overhead with raw model-run mode, timeout cancellation, semantic cross-message caching, an MCP-free per-run OpenClaw config projection, and bounded stage timing diagnostics.
