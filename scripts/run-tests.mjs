...
const requested = process.argv.slice(2).filter((arg) => arg !== "--").map((arg) => arg.trim()).filter(Boolean);
...
for (const file of files) {
  ...
  const result = spawnSync(process.execPath, ["--import", "tsx", "--test", file], {
    ...
    env: {
      ...process.env,
      OPENCLAW_HOME: process.env.OPENCLAW_HOME ?? resolve(cwd, ".tmp-openclaw-home"),
    },
  });
